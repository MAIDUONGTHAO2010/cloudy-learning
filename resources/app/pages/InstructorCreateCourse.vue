<template>
    <div class="min-h-screen bg-gray-50 text-gray-900">
        ...
        <main class="mx-auto max-w-3xl px-6 py-10 pb-32">

            <!-- Back -->
            <RouterLink to="/my-courses" class="mb-8 inline-flex items-center gap-2 text-sm text-gray-500 transition hover:text-gray-900">
                ← Back to My Courses
            </RouterLink>

            <h1 class="mb-8 text-2xl font-semibold text-gray-900">Create New Course</h1>
            ...
            <div class="mb-10 flex items-center gap-0">
                <template v-for="(step, idx) in steps" :key="step.key">
                    ...
                    <button
                        ...
                        :disabled="!canAccessStep(idx + 1)"
                        class="flex shrink-0 flex-col items-center gap-1.5 focus:outline-none disabled:cursor-not-allowed"
                    >
                        <div
                        ...
                        >
                            ...
                            <svg v-if="isStepComplete(idx + 1)" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" />
                            </svg>
                            <span v-else>{{ idx + 1 }}</span>
                        </div>
                        <span class="max-w-[80px] text-center text-xs leading-tight" :class="currentStep === idx + 1 ? 'text-gray-900' : 'text-gray-400'">
                            {{ step.label }}
                        </span>
                    </button>
                    ...
                    <div
                        ...
                        class="mx-1 h-0.5 flex-1 rounded-full transition-all"
                        :class="isStepComplete(idx + 1) ? 'bg-orange-500' : 'bg-gray-200'"
                    />
                </template>
            </div>

            <!-- ══════════════ STEP 1: Course Info ══════════════ -->
            <div v-if="currentStep === 1" class="rounded-2xl border border-gray-200 bg-white p-6">
                <h2 class="mb-5 text-lg font-semibold text-gray-900">Course Info</h2>

                <form @submit.prevent="submitCourseInfo" class="space-y-5">
                    <!-- Title -->
                    <div>
                        <label class="mb-1 flex items-center gap-1.5 text-xs text-gray-500">
                            Title <span class="text-rose-400">*</span>
                            <span v-if="courseForm.title.trim()" class="ml-auto text-[10px] font-semibold text-emerald-600">✓ Required field filled</span>
                        </label>
                        <input
                            v-model="courseForm.title"
                            type="text"
                            required
                            placeholder="e.g. Complete Vue 3 Masterclass"
                            class="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-2.5 text-sm text-gray-900 outline-none focus:border-orange-400 transition"
                            :class="courseForm.title.trim() ? 'border-emerald-500/50' : ''"
                        />
                    </div>
                    ...
                    <div>
                        ...
                        />
                        <div v-if="thumbnailUploading" class="mt-2">
                            <div class="h-1.5 w-full overflow-hidden rounded-full bg-gray-200">
                                <div class="h-full rounded-full bg-orange-500 transition-all duration-300" :style="`width:${thumbnailProgress}%`"></div>
                            </div>
                            <p class="mt-1 text-xs text-gray-500">Uploading {{ thumbnailProgress }}%…</p>
                        </div>
                    </div>

                    <p v-if="courseFormError" class="text-xs text-rose-400">{{ courseFormError }}</p>

                    <div class="flex justify-end pt-2">
                        <button
                            type="submit"
                            :disabled="!courseForm.title.trim() || courseFormSubmitting || thumbnailUploading"
                            class="inline-flex items-center gap-2 rounded-xl bg-[#1a1a4e] px-6 py-2.5 text-sm font-semibold text-white transition hover:bg-[#0f2460] disabled:opacity-50"
                        >
                            {{ courseFormSubmitting ? 'Creating…' : 'Create & Add Lessons →' }}
                        </button>
                    </div>
                </form>
            </div>
            ...
            <div v-else-if="currentStep === 2">
                <div class="mb-4 flex items-center justify-between">
                    <div>
                        <h2 class="text-lg font-semibold text-gray-900">Add Lessons</h2>
                        <p class="text-sm text-gray-500">Add at least one lesson to continue.</p>
                    </div>
                    <button
                        @click="openAddLesson"
                ...
                <div class="mt-6 flex items-center justify-between">
                    ...
                    <button
                        ...
                        class="rounded-xl bg-[#1a1a4e] px-6 py-2.5 text-sm font-semibold text-white transition hover:bg-[#0f2460] disabled:opacity-50"
                    >
                        Next: Add Quiz →
                    </button>
                </div>
            ...
            <div v-else-if="currentStep === 3">
                <div class="mb-4">
                    <h2 class="text-lg font-semibold text-gray-900">Quiz <span class="ml-2 rounded-full bg-gray-100 px-2.5 py-0.5 text-xs font-medium text-gray-500">Optional</span></h2>
                    <p class="mt-1 text-sm text-gray-500">Add quizzes to individual lessons, or skip and publish now.</p>
                </div>
                ...
                <div class="space-y-3">
                    <div
                        v-for="(lesson, idx) in lessons"
                        :key="lesson.id"
                        class="flex items-center justify-between rounded-2xl border border-gray-200 bg-white p-4"
                    >
                        <div class="flex items-center gap-3">
                            <span class="text-xs font-medium text-gray-400">#{{ idx + 1 }}</span>
                            <div>
                                <p class="text-sm font-semibold text-gray-900">{{ lesson.title }}</p>
                                <p v-if="lesson.has_quiz" class="text-xs text-purple-600">Quiz attached</p>
                                <p v-else class="text-xs text-gray-400">No quiz</p>
                            </div>
                        </div>
                        <button
                            @click="openQuiz(lesson)"
                            class="rounded-xl border px-3 py-1.5 text-xs transition"
                            :class="lesson.has_quiz ? 'border-purple-300 text-purple-700 hover:bg-purple-50' : 'border-gray-200 text-gray-600 hover:bg-gray-50'"
                        >
                            {{ lesson.has_quiz ? 'Manage Quiz' : '+ Add Quiz' }}
                        </button>
                    </div>
                </div>
                ...
                <div class="mt-6 flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-3">
                        <button
                            @click="finish(false)"
                            class="rounded-xl border border-gray-200 px-5 py-2.5 text-sm text-gray-600 transition hover:bg-gray-50"
                        >
                            Save as Draft
                        </button>
                        <button
        ...
        <!-- ── Sticky bottom publish bar (steps 2+) ── -->
        <div
            v-if="currentStep >= 2 && createdCourseId"
            class="fixed bottom-0 inset-x-0 border-t border-gray-200 bg-white/95 backdrop-blur-sm"
        >
            <div class="mx-auto flex max-w-3xl items-center justify-between gap-4 px-6 py-4">
                <div class="text-sm text-gray-500">
                    <span class="font-semibold text-gray-900">{{ courseForm.title }}</span>
                    <span class="mx-2 text-gray-300">·</span>
                    <span>{{ lessons.length }} lesson{{ lessons.length !== 1 ? 's' : '' }}</span>
                    <span class="mx-2 text-gray-300">·</span>
                    <span :class="isStepComplete(1) && isStepComplete(2) ? 'text-emerald-600' : 'text-gray-400'">
                        {{ isStepComplete(1) && isStepComplete(2) ? 'Ready to publish' : 'Add lessons to publish' }}
                    </span>
                </div>
                <button
                    @click="finish(true)"
            ...
            <div v-if="showLessonForm" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
                <div class="w-full max-w-lg rounded-2xl border border-gray-200 bg-white p-6 shadow-2xl overflow-y-auto max-h-[90vh]">
                    ...
                    <form @submit.prevent="submitLesson" class="space-y-4">
                        ...
                        <div class="flex items-center gap-2">
                            <input id="lf_active" v-model="lessonForm.is_active" type="checkbox" class="rounded border-gray-300 bg-white accent-orange-500" />
                            <label for="lf_active" class="text-sm text-gray-700">Active (visible to students)</label>
                        </div>
        ...
        <Teleport to="body">
            <div v-if="deletingLesson" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
                <div class="w-full max-w-sm rounded-2xl border border-gray-200 bg-white p-6 shadow-2xl">
                    ...
                    <p class="text-sm text-gray-500">"<span class="text-gray-900">{{ deletingLesson.title }}</span>" will be permanently deleted.</p>
                ...
                <div class="h-full w-full max-w-xl overflow-y-auto border-l border-gray-200 bg-white p-6 shadow-2xl">
                    ...
                    <template v-else>
                        ...
                        <template v-else>
                            ...
                            <div v-if="(quiz.questions?.length ?? 0) === 0" class="py-8 text-center text-sm text-gray-400">
                            ...
                            <div v-else class="space-y-4">
                                ...
                                >
                                    ...
                                    <template v-else>
                                        ...
                                        <!-- Options -->
                                        <div class="space-y-2">
                                            <div v-for="(opt, oIdx) in editForm.options" :key="opt.id" class="flex items-center gap-3">
...
interface QuestionOption { id: number; label: number; content: string; is_correct: boolean; }
interface Question { id: number; content: string; type: number; answer_type: number; options: QuestionOption[]; }
...
const steps = [
    ...
    { key: 'lessons', label: 'Add Lessons' },
    { key: 'quiz',    label: 'Quiz (optional)' },
];

const isStepComplete = (step: number): boolean => {
    if (step === 1) return createdCourseId.value !== null;
    if (step === 2) return lessons.value.length > 0;
    if (step === 3) return false; // optional — never "required complete"
    return false;
};
...
const canAccessStep = (step: number): boolean => {
    ...
    if (step === 3) return isStepComplete(1) && isStepComplete(2);
    return false;
};

const goToStep = (step: number) => {
    if (canAccessStep(step)) currentStep.value = step;
};

const stepCircleClass = (step: number): string => {
    if (isStepComplete(step)) return 'border-emerald-500 bg-emerald-500/20 text-emerald-400';
    if (currentStep.value === step) return 'border-orange-500 bg-orange-50 text-orange-500';
...
const handleThumbnailChange = async (event: Event) => {
    ...
    try {
        ...
        await axios.put(presign.upload_url, file, {
            ...
            onUploadProgress: (e) => {
                thumbnailProgress.value = e.total ? Math.round((e.loaded / e.total) * 100) : 0;
            },
        });
...
const submitCourseInfo = async () => {
    ...
    try {
        ...
        if (courseForm.value.category_id) payload.category_id = courseForm.value.category_id;
        if (courseForm.value.thumbnail)   payload.thumbnail    = courseForm.value.thumbnail;
...
// ── Lesson helpers ────────────────────────────────────────
const lessonBase = computed(() => `/api/instructor/courses/${createdCourseId.value}`);
...
const handleVideoChange = async (event: Event) => {
    ...
    try {
        const { data: presign } = await axios.post(`${lessonBase.value}/lessons/presign-video`, {
            file_name:    file.name,
            content_type: file.type,
            file_size:    file.size,
        });
...
const submitLesson = async () => {
    ...
    try {
        if (editingLesson.value) {
            const { data } = await axios.put(`${lessonBase.value}/lessons/${editingLesson.value.id}`, lessonForm.value);
            const idx = lessons.value.findIndex((l) => l.id === data.id);
            if (idx !== -1) lessons.value[idx] = { ...data, has_quiz: lessons.value[idx].has_quiz };
        } else {
            const { data } = await axios.post(`${lessonBase.value}/lessons`, lessonForm.value);
            lessons.value.push({ ...data, has_quiz: false });
        }
        closeLessonForm();
    } catch (e: any) {
        const errors = e?.response?.data?.errors;
        lessonFormError.value = errors
            ? (Object.values(errors).flat() as string[]).join(' ')
            : (e?.response?.data?.message ?? 'Error saving lesson.');
    } finally {
...
const deleteLesson = async () => {
    ...
    try {
        await axios.delete(`${lessonBase.value}/lessons/${deletingLesson.value.id}`);
...
const openQuiz = async (lesson: Lesson) => {
    ...
    try {
        const { data } = await axios.get(`${lessonBase.value}/lessons/${lesson.id}/quiz`);
        quiz.value          = data;
...
const createQuiz = async () => {
    ...
    quizFeedback.value      = null;
    try {
        const { data } = await axios.post(`${lessonBase.value}/lessons/${quizLesson.value.id}/quiz`);
        quiz.value          = data;
        currentQuizId.value = data?.id ?? null;
...
const deleteQuiz = async () => {
    ...
    quizActionLoading.value = true;
    try {
        await axios.delete(`${lessonBase.value}/quizzes/${currentQuizId.value}`);
        quiz.value           = null;
        currentQuizId.value  = null;
...
const submitNewQuestion = async () => {
    ...
    quizFeedback.value       = null;
    try {
        const { data } = await axios.post(`${lessonBase.value}/quizzes/${currentQuizId.value}/questions`, newQuestionForm.value);
...
const handleNewMediaChange = async (event: Event) => {
    ...
    try {
        const { data: presign } = await axios.post(`${lessonBase.value}/questions/presign-media`, {
            file_name:    file.name,
            content_type: file.type,
        });
        await axios.put(presign.upload_url, file, {
...
const cancelEdit = () => { editingQuestionId.value = null; };

const setCorrect = (idx: number) => {
    if (editForm.value.answer_type === 2) {
        editForm.value.options[idx].is_correct = !editForm.value.options[idx].is_correct;
    } else {
        editForm.value.options.forEach((o, i) => { o.is_correct = i === idx; });
    }
};
...
const saveQuestion = async (questionId: number) => {
    ...
    quizFeedback.value      = null;
    try {
        const { data } = await axios.put(`${lessonBase.value}/questions/${questionId}`, {
...
const deleteQuestion = async (questionId: number) => {
    ...
    quizFeedback.value      = null;
    try {
        await axios.delete(`${lessonBase.value}/questions/${questionId}`);
        if (quiz.value) quiz.value.questions = quiz.value.questions.filter((q) => q.id !== questionId);
    } catch (e: any) {
...
const finish = async (publish: boolean) => {
    ...
    if (publish) {
        ...
        try {
            await axios.put(`/api/instructor/courses/${createdCourseId.value}`, {
                title:       courseForm.value.title,
                description: courseForm.value.description || null,
                category_id: courseForm.value.category_id,
                is_active:   true,
            });
        } catch {
