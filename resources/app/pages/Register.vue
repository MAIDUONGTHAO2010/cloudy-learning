      ...
      <div class="hidden bg-gradient-to-br from-[#1a1a4e] via-[#0f2460] to-[#0c1d50] border-r border-white/10 lg:flex lg:flex-col lg:justify-between lg:px-14 lg:py-12">
        <div>
          ...
          <div class="mt-16 max-w-xl">
            <p class="text-sm font-semibold uppercase tracking-[0.35em] text-orange-400">Join today</p>
        ...
        <ul class="space-y-4">
          <li v-for="feature in features" :key="feature" class="flex items-center gap-3">
            <div class="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-orange-500/20 text-orange-400">
      ...
      <div class="relative flex items-center justify-center px-5 py-10 sm:px-8 lg:px-12">
        <div class="w-full max-w-xl rounded-[2rem] border border-white/10 bg-white/95 p-8 text-slate-900 shadow-[0_40px_120px_-50px_rgba(15,23,42,0.8)] backdrop-blur xl:p-10">
            ...
            <div>
              ...
              <div class="grid grid-cols-2 gap-3">
                ...
                >
                  ...
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-7 w-7" :class="form.role === 1 ? 'text-orange-600' : 'text-slate-400'" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                  ...
                  <span class="text-sm font-semibold" :class="form.role === 1 ? 'text-orange-700' : 'text-slate-600'">Student</span>
                  <span class="text-xs text-center" :class="form.role === 1 ? 'text-orange-500' : 'text-slate-400'">Browse &amp; learn courses</span>
                </label>

                <label
                  :class="form.role === 2
                    ? 'border-[#1a1a4e] bg-[#1a1a4e]/5 ring-2 ring-[#1a1a4e]/20'
                    : 'border-slate-200 hover:border-slate-300'"
                  class="flex cursor-pointer flex-col items-center gap-2 rounded-2xl border p-4 transition"
                >
                  <input type="radio" v-model.number="form.role" :value="2" class="sr-only" />
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-7 w-7" :class="form.role === 2 ? 'text-[#1a1a4e]' : 'text-slate-400'" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 6.042A8.967 8.967 0 006 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 016 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 016-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0018 18a8.967 8.967 0 00-6 2.292m0-14.25v14.25" />
                  </svg>
                  <span class="text-sm font-semibold" :class="form.role === 2 ? 'text-[#1a1a4e]' : 'text-slate-600'">Instructor</span>
                  <span class="text-xs text-center" :class="form.role === 2 ? 'text-[#0f2460]' : 'text-slate-400'">Create &amp; teach courses</span>
                </label>
              </div>
          ...
          <p class="mt-6 text-center text-sm text-slate-500">
            ...
            <RouterLink to="/login" class="font-semibold text-orange-500 hover:text-orange-600">
              Sign in
            </RouterLink>
          </p>
        </div>
