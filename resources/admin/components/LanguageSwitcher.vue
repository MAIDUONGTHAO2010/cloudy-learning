<template>
    <div class="relative" ref="dropdownRef">
        <button
            type="button"
            @click="open = !open"
            class="flex items-center gap-1.5 rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-600 shadow-sm transition hover:bg-slate-50"
        >
        ...
        <div
            v-if="open"
            class="absolute right-0 top-11 z-50 min-w-[140px] rounded-2xl border border-slate-200 bg-white py-1 shadow-xl"
        >
            <button
                ...
                class="flex w-full items-center gap-2.5 px-4 py-2.5 text-sm transition hover:bg-slate-50"
                :class="locale === loc.code ? 'text-blue-600 font-semibold' : 'text-slate-700'"
            >
                <span>{{ loc.flag }}</span>
                <span>{{ loc.label }}</span>
            </button>
